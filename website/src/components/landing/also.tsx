...
const POINTS = [
  ...
  },
  {
    ...
    body: "A local note of when each agent started in a workspace and how it ended — even while the window was closed — without reading a word it printed. Opt in, and every built-in agent reports its own tool calls and turns to it as metadata, its own settings untouched. The Changes list marks which files an agent reported writing, which were written while it ran a command, and which changed with no report at all. An experimental timeline shows the rest; ys activity export writes it out.",
  },
  {
