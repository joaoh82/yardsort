...
| ----------- | ------------------ | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Claude Code | 2.1.280            | Hooks defined in settings and plugins (`--bare` skips them); `--include-hook-events`; plugin directories            | Opt-in hook commands for prompt / tool / permission / session lifecycle       | **Shipped 2026-09-25**, fixtures recorded: see [11](11-agent-events-stage-2-claude.md).                                                |
| Codex       | 0.156.1            | `plugin` subcommand; hook trust (`--dangerously-bypass-hook-trust`) — a hook mechanism exists and is gated by trust | Hooks if their events cover tools; otherwise its session JSONL under fixtures | **Shipped 2026-09-25**, fixtures recorded; `notify` + session file, not hooks: see [12](12-agent-events-stage-2-codex.md).             |
| Grok        | 1.0.41             | `plugin` subcommand only                                                                                            | Lifecycle-only until a stable hook or log surface is found                    | **Shipped 2026-09-25**: a log surface was found, its own session directory: see [14](14-agent-events-stage-2-grok.md).                 |
| OpenCode    | 1.18.31            | `plugin <module>` install; `--pure` disables plugins                                                                | A managed plugin emitting tool lifecycle and usage                            | **Shipped 2026-09-25**, fixtures recorded; a plugin given per launch: see [13](13-agent-events-stage-2-opencode.md).                   |
| OMP         | 18.2.11            | `--hook=<file>` loads a hook/extension file; plugin install/link                                                    | Possibly a hook file; needs the extension API read                            | **Shipped 2026-09-25**, fixtures recorded; an extension given per launch with `-e`: see [15](15-agent-events-stage-2-pi-omp.md).       |
| Cursor      | 2026.09.23-86fc751 | `--plugin-dir`; `plugin` subcommand                                                                                 | Lifecycle-only                                                                | **Shipped 2026-09-25, unrecorded**; hooks through `--plugin-dir`, from the documentation: see [16](16-agent-events-stage-2-cursor.md). |
| Pi          | 0.87.1             | `PI_TELEMETRY` (its **own** install telemetry, unrelated to us — leave it alone); plugins                           | Lifecycle-only                                                                | **Shipped 2026-09-25**, partly recorded (no provider); OMP's extension, `-e`: see [15](15-agent-events-stage-2-pi-omp.md).             |
...
- **Next:** Stage 2, Claude Code first — record real hook payloads for 2.1.x as fixtures under
  ...
  `YARDSORT_RUN_ID` with the native session id as the fallback key. _Done 2026-09-25_, with
  two of those shapes changed on contact with the real thing — no installer, no receiver process —
  in [11-agent-events-stage-2-claude](11-agent-events-stage-2-claude.md).
